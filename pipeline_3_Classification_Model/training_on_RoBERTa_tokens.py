...
# 2. Load original CSV and align lengths
print("Loading dataset CSV...")
df = pd.read_csv(CSV_PATH, sep='|')
if len(df) != num_tokens:
    print(f"Trimming CSV from {len(df)} to {num_tokens} rows to match tokens...")
    df = df.iloc[:num_tokens].reset_index(drop=True)

# 3. Filter rare intents (need >=2 samples for stratification)
print("Filtering rare intent classes...")
...
if not rare_intents.empty:
    print(f"Dropping {rare_intents.sum()} samples from rare classes: {list(rare_intents.index)}")
mask = token_intents.isin(valid_intents)
# Apply mask to df and tokens
df = df[mask].reset_index(drop=True)
input_ids = input_ids[mask.values]
attention_mask = attention_mask[mask.values]
...
pd.DataFrame(rep).T.to_csv(os.path.join(MODELS_DIR,'eval_report.csv'))
